...
class IssueMaterial(models.Model):
    ...
    def _onchange_job_order_id(self):
        if not self.job_order_id:
            ...
            return
        bjo = self.job_order_id
        im_remarks = f"{bjo.remarks or ''}<br/><br/><b>Issue Material Remarks:</b><br/>"
        self.update({'process_rice_qty': bjo.process_rice_qty, 'remarks': im_remarks})
    ...
    def _onchange_source_location_ids_fetch_products(self) -> None:
        ...
        # Auto-fill first selected destination from header if available
        target_dest = self.dest_location_ids[0].id if self.dest_location_ids else False
    ...
    def action_confirm(self):
        for rec in self:
            if rec.state != 'draft':
                raise UserError(_("Only a Draft Issue Material can be confirmed."))
            if not rec.issue_line_ids:
                raise UserError(_("Please add raw materials before confirming."))
            if not rec.source_location_ids or not rec.dest_location_ids:
                raise UserError(_("Please select both Source and Destination locations before issuing."))
            ...
            with self.env.cr.savepoint():
                ...
                linked_record = rec._link_to_production_record()
                if (linked_record and linked_record.state == 'done' and linked_record.finished_material_weight > 0):
                    rec._complete_manufacturing_order(linked_record.finished_material_weight,
                                                      linked_record.dest_location_id or rec.dest_location_ids[:1])
        return True
    ...
    def _create_manufacturing_order(self) -> None:
        ...
        finished_product = self.job_order_id.product_id
        picking_type = self.env['stock.picking.type'].search(
            [('code', '=', MANUFACTURING_OPERATION_CODE), ('company_id', '=', self.env.company.id)], limit=1)
        planned_qty = self.process_rice_qty or self.total_issue_qty

        raw_move_commands = [(COMMAND_CREATE_NEW, 0, {
            'product_id': line.product_id.id,
            'product_uom': line.product_id.uom_id.id,
            'product_uom_qty': line.qty_mt,
            'location_id': line.shell_id.id,
        ...
        production = self.env['mrp.production'].create({
            'origin': self.name, 'product_id': finished_product.id, 'product_qty': planned_qty,
            'product_uom_id': finished_product.uom_id.id, 'picking_type_id': picking_type.id,
            'location_src_id': self.source_location_ids[:1].id, 'location_dest_id': self.dest_location_ids[:1].id,
            'move_raw_ids': raw_move_commands,
        })
        production.action_confirm()
        production.action_assign()
    ...
    def _complete_manufacturing_order(self, finished_qty, dest_location) -> Any:
        self.ensure_one()
        production = self.mrp_production_id
        if not production or production.state in ('done', 'cancel'): return True
        if dest_location:
            production.write({'location_dest_id': dest_location.id})
            production.move_finished_ids.write({'location_dest_id': dest_location.id})
        production.product_qty = finished_qty
        ...
        for move in production.move_raw_ids:
            issue_line = self.issue_line_ids.filtered(lambda line: line.product_id == move.product_id)[:1]
            if issue_line: move.quantity = issue_line.qty_mt
        return production.button_mark_done()

    def _link_to_production_record(self):
        self.ensure_one()
        production = self.env['production.record'].search(
            [('job_order_id', '=', self.job_order_id.id), ('issue_material_id', '=', False), ('state', '=', 'done')],
            limit=1, order='id desc')
        if production:
            production.write({'issue_material_id': self.id, 'raw_material_weight': self.total_issue_qty})
        return production or None

    def action_cancel(self) -> None:
        for rec in self:
            if rec.mrp_production_id and rec.mrp_production_id.state != 'cancel': rec.mrp_production_id.action_cancel()
            rec.state = 'cancel'
    ...
    def action_create_production_log_sheet(self) -> Dict[str, Any]:
        self.ensure_one()
        return {'type': 'ir.actions.act_window', 'name': 'Production Log Sheet', 'res_model': 'production.log.sheet',
                'view_mode': 'form', 'target': 'current',
                'context': {'default_issue_material_id': self.id, 'default_job_order_1_id': self.job_order_id.id,
                            'default_customer_1_id': self.job_order_id.partner_id.id,
                            'default_brand_id': self.job_order_id.brand_id.id}}

    def action_create_production_qc(self) -> Dict[str, Any]:
        self.ensure_one()
        return {'type': 'ir.actions.act_window', 'name': 'Production Quality Control',
                'res_model': 'production.quality.control', 'view_mode': 'form', 'target': 'current',
                'context': {'default_issue_material_id': self.id, 'default_job_order_id': self.job_order_id.id,
                            'default_customer_id': self.job_order_id.partner_id.id,
                            'default_brand_id': self.job_order_id.brand_id.id}}
...
class IssueMaterialLine(models.Model):
    ...
    def _compute_qty_on_hand(self) -> None:
        for line in self:
            if line.product_id and line.shell_id:
                product = line.product_id.with_context(location=line.shell_id.id)
                line.qty_on_hand = product.qty_available / 1000.0
            else:
                line.qty_on_hand = 0.0
    ...
    def _onchange_product_id_set_lot_shell(self):
        if self.product_id and self.issue_id.source_location_ids:
            quant = self.env['stock.quant'].search([('product_id', '=', self.product_id.id),
                                                    ('location_id', 'child_of', self.issue_id.source_location_ids.ids),
                                                    ('quantity', '>', 0)], limit=1, order='id asc')
            if quant:
                self.lot_id = quant.lot_id.id if quant.lot_id else False
                self.shell_id = quant.location_id.id
            else:
