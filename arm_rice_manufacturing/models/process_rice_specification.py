...
class ProcessRiceSpec(models.Model):
    ...
    # POINT: New Category Selection to filter Rice/Commodity
    commodity_group = fields.Selection([
        ('rice', 'Rice'),
        ('commodity', 'Commodity')
    ], string='Process Category', default='rice', required=True)
    ...
    # ==========================================
    # FIXED: Dynamic Selection Options Logic
    # ==========================================
    # 1. Main field (Ab compute hoga taake baqi logic broke na ho)
    rice_type = fields.Selection([
        ('irri', 'IRRI'),
        ('basmati', 'Basmati'),
        ('sesame seeds', 'SESAME SEEDS'),
        ('corn', 'CORN')
    ], string='Commodities Type', compute='_compute_rice_type', store=True, readonly=False, tracking=True)

    # 2. Sirf Rice ke liye (Isme sirf 2 options aayenge)
    rice_type_rice = fields.Selection([
        ('irri', 'IRRI'),
        ('basmati', 'Basmati')
    ], string='Rice Type')

    # 3. Sirf Commodity ke liye (Isme sirf 2 options aayenge)
    rice_type_commodity = fields.Selection([
        ('sesame seeds', 'SESAME SEEDS'),
        ('corn', 'CORN')
    ], string='Commodity Type')

    @api.depends('rice_type_rice', 'rice_type_commodity', 'commodity_group')
    def _compute_rice_type(self):
        for rec in self:
            if rec.commodity_group == 'rice':
                # Agar user ne select nahi kiya to default 'irri' rakho
                rec.rice_type = rec.rice_type_rice or 'irri'
            else:
                # Agar user ne select nahi kiya to default 'sesame seeds' rakho
                rec.rice_type = rec.rice_type_commodity or 'sesame seeds'

    @api.onchange('commodity_group')
    def _onchange_commodity_group(self):
        if self.commodity_group == 'rice':
            self.rice_type_rice = 'irri'
        else:
            self.rice_type_commodity = 'sesame seeds'
    ...
    def _onchange_product_id_map_spec(self) -> None:
        if self.rice_type not in (IRRI_RICE_TYPE, 'sesame seeds') and self.product_id:
