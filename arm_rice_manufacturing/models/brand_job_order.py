...
# Fields preserved when packing lines are re-synced to a changed product set.
# NOTE: 'net_weight', 'gross_weight' and 'total_empty_bag_weight' are computed
# on the line/header - never preserved, always re-derived.
...
class BrandJobOrder(models.Model):
    ...
    # IRRI only: all Process Rice products of this job - equal roles.
    ...
    # Brand - same per-type pattern as Process Rice, mapped from the PRS.
    ...
    # ==========================================================
    # PACKING LINES (the Packing tab): one line per Process Rice product.
    # The header packing fields below are COMPUTED MIRRORS of these lines,
    # kept for compatibility with the print report and downstream consumers.
    # ==========================================================
    ...
    # UoM comes straight from the PRS (single source of truth) - locked here too.
    ...
    # Mirrors without a line field anymore: derived from the remaining line
    # data (per-bag weight x bags; gross - tare with grams->kg conversion).
    ...
    def create(self, vals_list: List[Dict[str, Any]]) -> 'BrandJobOrder':
        ...
        for order in orders:
            # Server-side twin: the internal product follows the multi list for IRRI.
            if order.rice_type == IRRI_RICE_TYPE:
                order.product_id = order._get_derived_process_rice()

            # Server-side twin: packing lines follow the process rice products
            # when the caller (import/API) supplied none.
    ...
    def _compute_packing_mirrors(self) -> None:
        """Compatibility mirrors of the packing lines (report/downstream
        consumers): config from the first line; UoM straight from the PRS
        (live in cache); counts and weights as sums."""
        for rec in self:
            ...
            # Line gross is already kg (computed, lbs converted) - sum directly.
    ...
    def _compute_no_of_bags(self) -> None:
        """No of Bags = Planned Qty (MT) / bag weight.
        Editable: a manual override survives until qty or bag weight changes."""
        for line in self:
    ...
    # ==========================================================
    # IRRI MULTI-PRODUCT (internal derivation)
    # ==========================================================

    def _get_derived_process_rice(self) -> 'product.product':
        """Internal machinery: the product the Manufacturing Order produces.
        Deterministic (alphabetically first) - never shown to IRRI users."""
        self.ensure_one()
    ...
    def _get_job_process_rice_products(self) -> 'product.product':
        """Protocol 4.1 (DRY): the process rice products of this job - all tags
        for IRRI, the single product otherwise."""
        self.ensure_one()
    ...
    def _get_job_brands(self) -> 'master.brand':
        """Protocol 4.1 (DRY): the brands of this job - all tags for IRRI,
        the single selection otherwise."""
        self.ensure_one()
        if self.rice_type == IRRI_RICE_TYPE:
    ...
    def _onchange_process_rice_ids_sync_product(self) -> None:
        """IRRI: keep the hidden internal product in sync with the tags."""
        if self.rice_type == IRRI_RICE_TYPE:
            self.product_id = self._get_derived_process_rice()

    # ==========================================================
    # PACKING LINES SYNC (products + brand auto-mapping)
    # ==========================================================

    def _map_brands_to_lines(self, commands: List[Tuple[int, int, Dict[str, Any]]]) -> None:
        """Protocol 2.1 (SRP): assign the job's brands to new packing lines,
        round-robin (2 brands + 3 products -> B1, B2, B1). Existing lines
        with a user-set brand are left alone."""
        self.ensure_one()
        ...
        for command in commands:
            ...
            if line_vals.get('brand_id'):
                continue  # user-set or PRS-seeded: leave it
            line_vals['brand_id'] = brands[next_brand_index % len(brands)].id
            next_brand_index += 1

    def _sync_packing_lines_to_products(self) -> None:
        """Protocol 2.1 (SRP): rebuild the packing lines to match the job's
        process rice products. Existing data for surviving products is
        preserved; dropped products lose their lines; new products get
        blank lines, then the job's brands are auto-mapped onto the lines
        that carry none."""
        self.ensure_one()
    ...
    def _onchange_process_rice_sync_packing_lines(self) -> None:
        """The Packing tab follows the job's Process Rice: edit the products,
        the packing lines follow (typed values preserved, brands mapped)."""
        self._sync_packing_lines_to_products()

    @api.onchange('brand_ids', 'brand_id')
    def _onchange_brand_sync_packing_lines(self) -> None:
        """Brand selection follows into the packing lines: lines without a
        brand (or ones whose brand is no longer among the job's brands)
        receive the round-robin mapping."""
        self.ensure_one()
        brands = self._get_job_brands()
        ...
        # Re-map brands onto the surviving lines that need one.
    ...
    def _onchange_process_rice_spec_id(self) -> None:
        ...
        # brands round-robin mapped onto the lines. (pp_bag_uom is seeded for
        # instant UI feedback; the line compute enforces it server-side.)
...
class BrandJobOrderPackingLine(models.Model):
    ...
    # Percentage of empty bags shipped with this product's packing.
    # FIX: promoted from Char to Float - the user enters a percentage value.
    ...
    # The UoM is FORCED from the PRS: computed, stored, readonly - the user
    # cannot change it, and the Kgs/Lbs column visibility follows the parent
    # mirror so it re-evaluates instantly.
    pp_bag_uom = fields.Selection(
        [('kg', 'Kgs'), ('lb', 'Lbs')],
    ...
    # Gross per bag, always in KG: (bag tare converted to kg) + bag weight
    # (lbs converted to kg when the spec's UoM is Lbs).
    ...
    def _compute_pp_bag_uom(self) -> None:
        """The line's UoM follows the PRS's choice - one source of truth.
        Fallback 'kg' for jobs without a spec (standalone BJOs)."""
        for line in self:
    ...
    def _get_gross_weight_value(self) -> float:
        """Protocol 4.1 (DRY): the gross math, shared by the stored compute
        (server truth after save) and the onchange (live UI while typing)."""
    ...
    def _compute_gross_weight(self) -> None:
        """Server-side truth: runs on save/flush and on the upgrade sweep."""
        for line in self:
    ...
    def _onchange_packing_inputs_update_gross(self) -> None:
        """UI twin: gross updates LIVE while typing in the editable list.
        Stored computes only re-fire after save; this onchange pushes the
        partial result (one field filled, other still 0) to the screen
        immediately."""
        for line in self:
            line.gross_weight = line._get_gross_weight_value()
